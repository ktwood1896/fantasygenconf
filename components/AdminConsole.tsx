...
import { saveAdminResult } from "@/app/actions"; // We will create this next

export default function AdminConsole({ roster, currentResults }: { roster: any[], currentResults: any[] }) {
  // Initialize state with the existing results (so you can resume if you refresh)
  ...
  // --- HANDLERS (Same logic as DraftBoard) ---
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <section className="bg-slate-800 p-6 rounded-xl border border-slate-700">
        <h2 className="text-xl font-bold text-white mb-4">Tie Breakers</h2>
      ...
      {/* 2. AUXILIARIES */}
      <section className="bg-slate-800 p-6 rounded-xl border border-slate-700">
        <h2 className="text-xl font-bold text-white mb-4">Auxiliary Presidencies</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {["Relief Society", "Young Women", "Young Men", "Primary", "Sunday School", "Presiding Bishopric"].map(org => {
             // Convert "Relief Society" -> "relief_society" for the ID
             const catKey = org.toLowerCase().replace(" ", "_"); 
             // Note: "Presiding Bishopric" -> "presiding_bishopric", but your DB expects "bishopric". 
             // Let's manually fix the key for Bishopric to match your DraftBoard logic:
      ...
      {/* 3. GA 70s */}
      <section className="bg-slate-800 p-6 rounded-xl border border-slate-700">
        <h2 className="text-xl font-bold text-white mb-4">General Authority Seventies</h2>
      ...
      {/* THE BIG RED BUTTON */}
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-slate-900 border-t border-slate-700 flex justify-center shadow-2xl z-50">
        <button
          onClick={handlePublish}
          disabled={saving}
          className="bg-green-600 text-white font-bold py-3 px-12 rounded-full shadow-lg hover:bg-green-500 hover:scale-105 transition disabled:opacity-50 text-lg"
        >
